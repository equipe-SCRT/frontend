...
const VoluntariosCadastro = () => {
  ...
  const handleSaveClick = () => {
    setIdAlt(editedRowData.id);
    setNomeAlt(editedRowData.nome);
    setEmailAlt(editedRowData.email);
    if (editedRowData.tipoUsuario == "Administrador") {
      setTipoUsuarioAlt(1);
    } else {
      setTipoUsuarioAlt(0);
    }
    ...
    alert("Puta que pariu:" + usuarioAlterado.id + usuarioAlterado.nome + usuarioAlterado.email + usuarioAlterado.tipoUsuario);
    const response = api.patch(`/usuarios/atualizar-usuario`, usuarioAlterado)
    setEditMode(false);
